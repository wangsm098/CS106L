...
struct Course {
    ...
    /**
     * You don't have to ignore this anymore! We're defining the `==` operator
     * for the Course struct.
     */
...
class CourseDatabase {
  public:
    ...
    std::optional<Course*> find_course(std::string course_title) {
        /* STUDENT_TODO: Implement this method! You will need to change the
         * return type. */
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 2) {
        ...
        /********************************************************
        STUDENT_TODO: Populate the output string with the right information to
        print Please pay special attention to the README here
        ********************************************************/

        std::string output = course.and_then([] (Course* course) -> std::optional<std::string>{
            std::optional<std::string> res{"Found course: " + course->title + "," + course->number_of_units + "," + course->quarter};
            return res;
        }).or_else([] () -> std::optional<std::string> {
            std::optional<std::string> res{"Course not found."};
            return res;
        }).value(); 
